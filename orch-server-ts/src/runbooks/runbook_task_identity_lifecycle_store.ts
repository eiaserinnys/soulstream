...
import { bindingRows } from "./runbook_task_identity_queries.js";
import {
  appendRunbookEvent,
  findOperation,
...
import { mountBindingsEqual } from "./runbook_task_mount_reconciliation.js";
...
export async function persistTaskProjectMove(
  ...
  await sql.begin(async (transaction) => {
    ...
    if (locked.folderId !== input.sourceFolderId) {
      throw new Error(`task identity source folder changed: ${input.binding.runbookId}`);
    }
    if (locked.runbookVersion !== input.binding.runbookVersion) {
    ...
    const updated = await transaction<readonly { id: string }[]>`
      ...
      RETURNING id
    `;
    if (!updated[0]) throw new Error(`runbook version conflict: ${input.binding.runbookId}`);
    await insertRunbookOperation(transaction, {
      ...
      reason: "move runbook task identity between projects",
    });
  });
}
