...
export async function moveRunbookTaskIdentity(input: {
  ...
  if (move.boardItem.itemType !== "runbook") {
    throw new Error(`task identity move requires runbook: ${move.boardItem.itemType}`);
  }
  if (move.targetScope.containerKind !== "folder"
    || move.targetScope.containerId !== move.targetScope.folderId) {
    throw new Error("runbook task identity can only move between project folders");
  }
  const idempotent = await input.config.repository.findMutationByIdempotencyKey(
    move.idempotencyKey,
  );
  if (idempotent) {
    const current = await input.config.repository.findByRunbookId(move.boardItem.itemId);
  ...
  const binding = await input.config.repository.findByRunbookId(move.boardItem.itemId);
  ...
  const moved = await input.config.board.withRunbookBoardMoveApplication(
    move,
