...
export interface RunbookTaskIdentityHostRouteOptions {
  service: Pick<
...
export function registerRunbookTaskIdentityHostRoute(
  app: FastifyInstance,
  options: RunbookTaskIdentityHostRouteOptions,
): void {
  app.post<{ Params: { operation: string } }>(
    "/api/runbook-task-identities/host/:operation",
    async (request, reply) => {
      ...
      try {
        ...
        if (operation === "backfill-legacy") {
          return reply.send(await options.service.backfillLegacyRunbook({
            runbookId: input.runbook_id as string,
            existingPageId: input.page_id as string | undefined,
            ...common,
          }));
        }
        return reply.send(await options.service.mutateFromRunbook({
          runbookId: input.runbook_id as string,
