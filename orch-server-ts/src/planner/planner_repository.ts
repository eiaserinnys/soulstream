...
type PlannerKind = "today" | "project";

interface PlannerPayloadRow extends Record<string, unknown> {
  payload: PlannerTodayDto | PlannerProjectDto | null;
}
...
export class PlannerRepository implements PlannerReadProvider {
  ...
  async getToday(date: string): Promise<PlannerTodayDto | null> {
    return await this.read("today", date) as PlannerTodayDto | null;
  }

  async getProject(pageId: string): Promise<PlannerProjectDto | null> {
    return await this.read("project", pageId) as PlannerProjectDto | null;
  }

  private async read(
    kind: PlannerKind,
    selector: string,
  ): Promise<PlannerTodayDto | PlannerProjectDto | null> {
    const sql = await this.resolver.resolveSql();
    const rows = await plannerQuery(sql, kind, selector);
    return rows[0]?.payload ?? null;
  }
}

async function plannerQuery(
  sql: LivePostgresSql,
  kind: PlannerKind,
  selector: string,
  ...
  return await sql`
    ...
    starred_projects AS (
      ...
      WHERE p.archived = FALSE
        ...
        AND (p.metadata->>'starred')::boolean = TRUE
    ),
    task_rows AS (
      SELECT mounted.id AS page_id,
    ...
    relevant_page_ids AS (
      ...
      UNION SELECT id FROM mounted_pages
      UNION SELECT id FROM starred_projects WHERE ${kind} = 'today'
      UNION
    ...
    block_payloads AS (
      ...
      WHERE b.page_id IN (
        SELECT id FROM root_page
        UNION SELECT page_id FROM task_rows
      )
    ),
    page_blocks AS (
    ...
    task_runbook_ids AS (
      SELECT DISTINCT runbook_id FROM task_rows
    ),
    ...
    runbook_sessions AS (
      SELECT item.container_id AS runbook_id,
             jsonb_agg(
               jsonb_build_object(
      ...
      JOIN task_runbook_ids task_runbook ON task_runbook.runbook_id = item.container_id
    ...
    payload AS (
      SELECT CASE
        ...
        ELSE jsonb_build_object(
          ...
          'documents', COALESCE((
            SELECT jsonb_agg(page_payload.payload ORDER BY mounted.mount_position DESC, page.id)
            FROM mounted_pages mounted
            JOIN mounted_kinds mounted_kind ON mounted_kind.page_id = mounted.id
            JOIN pages page ON page.id = mounted.id
            JOIN page_payloads page_payload ON page_payload.id = page.id
            WHERE mounted_kind.runbook_id IS NULL
          ), '[]'::jsonb)
        )
      END AS payload
