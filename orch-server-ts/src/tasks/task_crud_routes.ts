...
import type { TaskRouteOptions, TaskSnapshot } from "./task_route_types.js";
...
const routes: readonly CrudRoute[] = [
  { operation: "create_section", path: "/api/tasks/:task_id/sections" },
  { operation: "update_section", path: "/api/tasks/:task_id/sections/:section_id" },
  { operation: "move_section", path: "/api/tasks/:task_id/sections/:section_id/move" },
  { operation: "archive_section", path: "/api/tasks/:task_id/sections/:section_id/archive" },
  { operation: "create_item", path: "/api/tasks/:task_id/sections/:section_id/items" },
  { operation: "update_item", path: "/api/tasks/:task_id/items/:item_id" },
  { operation: "move_item", path: "/api/tasks/:task_id/items/:item_id/move" },
  { operation: "archive_item", path: "/api/tasks/:task_id/items/:item_id/archive" },
];
...
export function registerTaskCrudRoutes(
  ...
  for (const route of routes) {
    app.post<{ Params: Params }>(route.path, async (request, reply) => {
      ...
      return proxyTaskMutation(request, reply, options, actorResult.actorSessionId, {
...
function actorSessionFor(
  ...
  if (operation === "create_section") {
    return actorResult(resolveTaskActorSessionId(snapshot), "Task");
  }
  if (operation.includes("section") || operation === "create_item") {
    ...
    if (snapshotSection(snapshot, sectionId) === undefined) {
      return { ok: false, statusCode: 404, message: "Task section not found" };
    }
    return actorResult(resolveSectionActorSessionId(snapshot, sectionId), "Task section");
  }
  const itemId = params.item_id ?? "";
  if (snapshotItem(snapshot, itemId) === undefined) {
    return { ok: false, statusCode: 404, message: "Task item not found" };
  }
  return actorResult(resolveItemActorSessionId(snapshot, itemId), "Task item");
}
...
function upstreamPath(operation: CrudOperation, params: Params): string {
  const task = encodeURIComponent(params.task_id);
  const section = encodeURIComponent(params.section_id ?? "");
  const item = encodeURIComponent(params.item_id ?? "");
  switch (operation) {
    case "create_section": return `/api/tasks/${task}/sections`;
    case "update_section": return `/api/tasks/${task}/sections/${section}`;
    case "move_section": return `/api/tasks/${task}/sections/${section}/move`;
    case "archive_section": return `/api/tasks/${task}/sections/${section}/archive`;
    case "create_item": return `/api/tasks/${task}/sections/${section}/items`;
    case "update_item": return `/api/tasks/${task}/items/${item}`;
    case "move_item": return `/api/tasks/${task}/items/${item}/move`;
    case "archive_item": return `/api/tasks/${task}/items/${item}/archive`;
  }
}
