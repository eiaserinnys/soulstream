...
describe("control-plane host routes", () => {
  ...
  it("routes exact execution recovery and legacy terminal inventory without collapsing identity", async () => {
    const activeGenerationsForExecution = vi.fn(async () => []);
    const terminalForNode = vi.fn(async () => []);
    const app = Fastify();
    apps.push(app);
    registerPersistenceHostRoutes(app, {
      authBearerToken: token,
    ...
    const exact = await app.inject({
      method: "POST",
      url: "/api/claude-runtime/host/active_background_generations_for_execution",
      headers: { authorization: `Bearer ${token}` },
      payload: {
        args: ["node-1", "session-1", "registration-1", "command-1", 1_000],
      },
    });
    const legacy = await app.inject({
      method: "POST",
      url: "/api/claude-runtime/host/terminal_background_tasks_for_node",
      headers: { authorization: `Bearer ${token}` },
      payload: { args: ["node-1", 1_000] },
    });
    ...
    expect(terminalForNode).toHaveBeenCalledWith("node-1", 1_000);
  });
