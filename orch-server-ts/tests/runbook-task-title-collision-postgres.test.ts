...
describe("Runbook task title collision policy", () => {
  ...
  it("promotes the canonical standalone page in place and preserves its content and daily mount", async () => {
    ...
    expect(created).toMatchObject({ id: pageId, pageId, runbookId: pageId, projectPageId });
    const rows = await harness.sql<Array<{
      ...
      SELECT page.title AS page_title, runbook.title AS runbook_title,
        (SELECT COUNT(*)::int FROM blocks
          WHERE page_id = ${pageId} AND text_plain = '기존 본문') AS body_count,
        (SELECT COUNT(*)::int FROM blocks
          WHERE page_id = ${pageId} AND block_type = 'runbook_ref'
      ...
      FROM pages page
      JOIN runbooks runbook ON runbook.task_page_id = page.id
      WHERE page.id = ${pageId}
    `;
    expect(rows[0]).toEqual({
      page_title: "아리엘라 EA 대사 작업",
      runbook_title: "아리엘라 EA 대사 작업",
      body_count: 1,
      reference_count: 1,
      daily_mount_count: 1,
  ...
  it("rolls back the page promotion and project mount when a later runbook write fails", async () => {
    ...
    const rows = await harness.sql<Array<{
      ...
      SELECT
        ...
        (SELECT COUNT(*)::int FROM blocks
          WHERE page_id = ${pageId} AND block_type = 'runbook_ref') AS reference_count,
        (SELECT COUNT(*)::int FROM runbooks WHERE task_page_id = ${pageId}) AS runbook_count,
  ...
  it("rejects archived, daily, project-root, and explicit-id collisions instead of repurposing them", async () => {
    ...
    await expect(service.create({
      title: "명시 ID 충돌",
      folderId,
      runbookId: "00000000-0000-4000-8000-0000000000c6",
      actor,
    ...
    const rows = await harness.sql<Array<{ runbook_count: number }>>`
      SELECT COUNT(*)::int AS runbook_count FROM runbooks
      WHERE task_page_id IN (
        'page-title-archived', 'daily-title-role', ${projectPageId},
        ${standalonePageId}, ${mountedPageId}
      )
    `;
    expect(rows[0]?.runbook_count).toBe(0);
  });
  ...
  async function identityCounts(taskId: string, projectPageId: string) {
    const rows = await harness.sql<Array<{
      ...
      SELECT
        ...
        (SELECT COUNT(*)::int FROM runbooks WHERE task_page_id = ${taskId}) AS runbook_count,
        (SELECT COUNT(*)::int FROM block_links link
          JOIN blocks source ON source.id = link.source_block_id
