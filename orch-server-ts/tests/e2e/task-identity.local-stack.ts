...
import { registerTaskIdentityHostRoute } from "../../src/tasks/task_identity_host_route.js";
import type {
...
try {
  ...
  const identity = new TaskIdentityService({
    board: localBoardPort(),
    repository: new SqlTaskIdentityRepository(resolver),
  ...
  const result = await runPlaywrightLifecycle<LocalStackResult, LocalBrowser>({
    lockName: "pr-ae-task-identity-local-stack",
    timeoutMs: 120_000,
  }, async ({ browser }) => {
    ...
    const mcpResponse = await fetch(`${baseUrl}/api/task-identities/host/create`, {
      method: "POST",
      headers: {
...
function operationIdFactory(): () => string {
  const ids = [
    "pr-ae-task-operation-v1",
    "pr-ae-page-operation-v1",
    "pr-ae-task-operation-mcp",
...
interface LocalStackResult {
  v1: {
    ...
    planner: { tasks: { items: Array<{ page: { id: string }; task_id: string }> } };
  };
...
function localBoardPort(): TaskIdentityBoardPort {
  const boardItems: Array<TaskIdentityBoardApplication["replica"]["boardItems"][number]> = [];
  return {
