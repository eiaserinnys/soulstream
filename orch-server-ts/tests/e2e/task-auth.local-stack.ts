...
try {
  ...
  app.get("/", async (_request, reply) =>
    reply.type("text/html").send("<main>Task auth lifecycle</main>"));
  ...
  registerTaskCreateRoute(app, {
    ...
    taskIdentityService: {
      ...
      mutateFromTask: async () => {
        throw new Error("not used");
      },
      backfillLegacyTask: async () => {
        throw new Error("not used");
      },
    },
  });
  ...
  const result = await runPlaywrightLifecycle<LifecycleResult, LocalBrowser>({
    lockName: "pr-br-task-auth-local-stack",
    timeoutMs: 120_000,
  }, async ({ browser }) => {
  ...
  console.log(JSON.stringify({
    ok: true,
    scenario: "auth status 200 and POST /api/tasks 201 with one JWT verification per request",
    verificationCount,
