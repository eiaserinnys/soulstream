...
describe("Node WS frame controller primitive", () => {
  ...
  it("registers runner process nodes without lease compatibility conditions", () => {
    const registry = new InMemoryNodeRegistry();
    const controller = new NodeWsFrameController({ registry });
    ...
    expect(registry.getNodeState("runner-node")?.capabilities).toEqual({
      runner_process_v1: true,
    });
  });
