...
try {
  const rows = await sql<CandidateRow[]>`
    ...
    FROM runbooks r
    LEFT JOIN blocks b
      ON b.block_type = 'runbook_ref'
     AND b.properties->>'runbookId' = r.id
  ...
  if (!apply) {
    ...
    if (ambiguous.length > 0) {
      throw new Error("ambiguous primary runbook_ref pages must be resolved before apply");
    }
    ...
    for (const row of rows) {
      ...
      const response = await fetch(
        `${orchBaseUrl}/api/runbook-task-identities/host/backfill-legacy`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            actor_kind: "system",
            idempotency_key: `backfill-task-identity:${row.runbook_id}`,
            reason: "PR-AE runbook task identity backfill",
          }),
        },
      );
      if (!response.ok) {
        throw new Error(
          `backfill failed for ${row.runbook_id}: ${response.status} ${await response.text()}`,
        );
      }
      const result = await response.json() as { runbookId: string; pageId: string };
