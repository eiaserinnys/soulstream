...
- `src/upstream.schema.json` — JSON Schema Draft 2020-12. 메시지 정의 112개 $defs (wire 53 + SSE event 59).
  - wire 메시지 53종
  - SSE event payload 59종 (`event` 메시지의 `event` 키 안에 packed — canonical 58종 + production-gated 읽기 호환 `runbook_updated` 1종)
