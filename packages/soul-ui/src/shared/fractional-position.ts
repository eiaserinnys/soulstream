...
const MIDDLE_DIGIT = POSITION_ALPHABET[Math.floor(POSITION_ALPHABET.length / 2)];
...
export function generateKeyBetween(
  ...
  for (let index = 0; index < lower.length || index < upper.length; index += 1) {
    ...
    const lowerDigit = digitIndex(lower[index]);
    const upperDigit = digitIndex(upper[index]);
