...
import { applyRunbookMutationOptimistically } from "./runbook-mutations";

describe("applyRunbookMutationOptimistically", () => {
  it("creates and edits one section without mutating the input snapshot", () => {
    const before = snapshot();
    const created = applyRunbookMutationOptimistically(before, {
      kind: "create_section",
      runbookId: "rb-1",
      sectionId: "sec-3",
      title: "새 섹션",
    ...
    const edited = applyRunbookMutationOptimistically(created, {
      kind: "update_section",
      runbookId: "rb-1",
      sectionId: "sec-3",
      expectedVersion: 1,
  ...
  it("moves sections and items using the requested sibling bounds", () => {
    ...
    const movedSection = applyRunbookMutationOptimistically(before, {
      kind: "move_section",
      runbookId: "rb-1",
      sectionId: "sec-2",
      expectedVersion: 1,
    ...
    const movedItem = applyRunbookMutationOptimistically(movedSection, {
      kind: "move_item",
      runbookId: "rb-1",
      itemId: "item-2",
      sectionId: "sec-1",
  ...
  it("archives only the requested target and preserves sibling rows", () => {
    const before = snapshot();
    const withoutItem = applyRunbookMutationOptimistically(before, {
      kind: "archive_item",
      runbookId: "rb-1",
      itemId: "item-1",
      expectedVersion: 1,
    ...
    const withoutSection = applyRunbookMutationOptimistically(withoutItem, {
...
function snapshot(): RunbookSnapshot {
  const common = {
    archived: false,
    version: 1,
  ...
  return {
    ...
    sections: [
      { ...common, ...assignee, id: "sec-1", runbook_id: "rb-1", position_key: "a", title: "One" },
      { ...common, ...assignee, id: "sec-2", runbook_id: "rb-1", position_key: "b", title: "Two" },
    ],
    items: [
