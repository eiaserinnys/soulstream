...
export interface RunbookAttentionCounts {
  todo: number;
  review: number;
  total: number;
}
...
export function isTodoItem(item: RunbookOverviewItem): boolean {
  return item.effective_assignee_kind === "human" &&
    (item.status === "pending" || item.status === "in_progress");
}

export function countAttentionItems(items: RunbookOverviewItem[]): RunbookAttentionCounts {
  const todo = items.filter(isTodoItem).length;
  const review = items.filter((item) => isRunbookItemReview(item.status)).length;
  return { todo, review, total: todo + review };
}

export function runbookAttentionCounts(
  group: RunbookOverviewGroup,
  myTurnItems: RunbookOverviewItem[],
): RunbookAttentionCounts {
  return countAttentionItems(
    myTurnItems.filter((item) => item.runbook_id === group.runbook_id),
  );
}

export function progressText(group: RunbookOverviewGroup): string {
...
function OpenRunbookBoardButton({
  group,
  onOpenRunbookBoard,
}: {
  group: RunbookOverviewGroup;
...
function RunbookAttentionMark({ counts }: { counts: RunbookAttentionCounts }) {
  if (counts.total <= 0) {
    return (
      <span
        data-testid="runbook-overview-runbook-attention-placeholder"
  ...
  return (
    <span
      data-testid="runbook-overview-runbook-attention"
...
export function RunbookListRow({
  group,
  attention,
  selected,
  onSelect,
  ...
  group: RunbookOverviewGroup;
  attention: RunbookAttentionCounts;
  selected: boolean;
  onSelect: () => void;
  onOpenRunbookBoard: (group: RunbookOverviewGroup) => void;
  onStatusChanged: () => Promise<void>;
  ...
  return (
    <div
      ...
      data-testid="runbook-overview-runbook-row"
      data-runbook-id={group.runbook_id}
      className={cn(
    ...
    >
      <RunbookAttentionMark counts={attention} />
...
export function RunbookItemsPane({
  group,
  onOpenBoard,
  onStatusChanged,
}: {
  group: RunbookOverviewGroup | null;
  onOpenBoard: (item: RunbookOverviewItem) => void;
  onStatusChanged: () => Promise<void>;
}) {
  if (!group) {
    return (
      <div
        data-testid="runbook-overview-selected-empty"
  ...
  return (
    <section
      data-testid="runbook-overview-selected-items"
      className="flex h-full min-h-0 flex-col gap-3"
    >
      <div className="flex min-w-0 items-center gap-2">
        <BookOpenCheck className="h-4 w-4 shrink-0 text-accent-blue" />
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-sm font-semibold leading-5">{group.runbook_title}</h2>
      ...
      <div
        data-testid="runbook-overview-selected-items-scroll"
