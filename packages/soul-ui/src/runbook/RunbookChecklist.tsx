...
import type { RunbookChecklistMutation } from "../stores/runbook-mutations";
import {
  type RunbookItemRow,
...
export function RunbookChecklist({
  ...
  const mutateChecklist = useRunbookStore((state) => state.mutateChecklist);
  ...
  async function mutate(input: RunbookChecklistMutation, onSuccess?: () => void) {
  ...
  function sectionActions(section: RunbookSectionRow, index: number): RowAction[] {
  ...
  function itemActions(
    ...
    item: RunbookItemRow,
    siblings: readonly RunbookItemRow[],
    index: number,
  ): RowAction[] {
  ...
  return (
    <div className="space-y-5">
      {sections.map((section, sectionIndex) => {
        ...
        return (
          <section key={section.id} data-testid="runbook-section" className="group/section">
            {editingSection ? (
              <SectionTitleForm
...
function keyFor(operation: string, targetId: string, version?: number): string {
  ...
  return `runbook:${operation}:${targetId}:${versionPart}:${newId()}`;
}
