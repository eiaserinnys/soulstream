 ...
 *
 * SessionList | VerticalSplitPane(FolderContents + centerPanelBottom) | RightPanel (Detail + Chat) 구성.
...
import { StorageModeToggleCompact } from "./StorageModeToggle";
...
export interface DashboardLayoutProps {
  /** 좌측 패널 하단에 추가할 컨텐츠 (예: NodePanel) */
  leftPanelBottom?: ReactNode;
  /** 좌측 패널 상/하 비율. 기본 [1, 0] (FolderTree만) */
  leftPanelRatio?: [number, number];
  /** 중앙 패널 하단 컨텐츠. undefined이면 비어있음 */
  centerPanelBottom?: ReactNode;
  /** 헤더 제목. 기본 "Soul Dashboard" */
  headerTitle?: string;
  /** 헤더 우측 추가 위젯 */
  headerExtra?: ReactNode;
  /** 숨길 기본 헤더 위젯 */
  hideFeatures?: ('search' | 'config' | 'storageToggle' | 'themeToggle')[];
  /** 새 세션 모달 대체. undefined이면 표시 안 함, null이면 기본 비활성 */
    ...
    >
      ...
      <header className="flex items-center justify-between px-4 h-10 border-b border-border bg-popover shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[14px] font-semibold text-muted-foreground tracking-[0.02em]">
            {props.headerTitle ?? "Soul Dashboard"}
          </span>
        </div>
        {!isMobile && (
          <div className="flex items-center gap-2">
            {props.headerExtra}
            {!hiddenFeatures.has('search') && (
              <Button variant="ghost" size="icon" onClick={props.onSearchClick}>
                <Search className="h-4 w-4" />
              </Button>
            )}
            {!hiddenFeatures.has('themeToggle') && <ThemeToggle />}
            {!hiddenFeatures.has('config') && props.onConfigClick && (
              <ConfigButton onClick={props.onConfigClick} />
            )}
            {!hiddenFeatures.has('storageToggle') && <StorageModeToggleCompact />}
      ...
      {isMobile ? (
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left: Folder Tree + optional bottom panel */}
          <aside
          ...
          >
            <div
              ...
              style={{
                flexBasis: props.leftPanelRatio
                  ? `${props.leftPanelRatio[0] / (props.leftPanelRatio[0] + props.leftPanelRatio[1]) * 100}%`
                  : '100%',
              }}
            >
              <FolderTree />
            ...
            {props.leftPanelBottom && (
              <div
                ...
                style={{
                  flexBasis: `${(props.leftPanelRatio?.[1] ?? 0) / ((props.leftPanelRatio?.[0] ?? 1) + (props.leftPanelRatio?.[1] ?? 0)) * 100}%`,
                }}
              >
                {props.leftPanelBottom}
              </div>
          ...
          {/* Center: SessionsTopBar + VerticalSplitPane (top: sessions, bottom: configurable) */}
          <main
          ...
          >
            <SessionsTopBar />
            {props.centerPanelBottom !== undefined ? (
              <VerticalSplitPane
                className="flex-1 overflow-hidden"
                top={<FolderContents />}
                bottom={props.centerPanelBottom}
              />
            ) : (
      ...
      {/* New Session Modal (optional) */}
      {props.newSessionDialog}
    </div>
  );
}
