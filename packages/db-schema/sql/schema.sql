...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    display_name_search_key TEXT GENERATED ALWAYS AS (session_search_compact(display_name)) STORED,
    prompt_search_key       TEXT GENERATED ALWAYS AS (session_search_compact(prompt)) STORED
);
...
-- Canonical Claude background execution generations. The legacy table above
-- remains the rolling-deploy projection consumed by unchanged workers.
...
-- PostgreSQL-owned global settings. Policy writers use version as a CAS token.
...
CREATE TABLE IF NOT EXISTS worktrees (
    ...
    CONSTRAINT worktrees_state_check CHECK (state IN ('ready', 'removing', 'removed')),
    CONSTRAINT worktrees_setup_mode_check CHECK (setup_mode IN ('none', 'shared_dependencies')),
    CONSTRAINT worktrees_setup_status_check CHECK (setup_status IN ('not_requested', 'ready', 'failed')),
    CONSTRAINT worktrees_managed_paths_array_check CHECK (jsonb_typeof(managed_paths) = 'array')
);

CREATE UNIQUE INDEX IF NOT EXISTS uq_worktrees_live_branch
    ON worktrees(node_id, repo_id, branch) WHERE state <> 'removed';
CREATE UNIQUE INDEX IF NOT EXISTS uq_worktrees_live_path
    ON worktrees(node_id, canonical_path) WHERE state <> 'removed';
...
-- Central recurring jobs. A run reserves its stable session ID before a node
-- creates it, so recurring_job_runs.session_id intentionally has no sessions FK.
