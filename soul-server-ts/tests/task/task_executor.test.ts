...
function makeMocks() {
  ...
  return {
    ...
    recordExecutionRegistrationAndWaitForApplication:
      persistenceDouble.recordExecutionRegistrationAndWaitForApplication,
    enqueueMetadataEffect,
    handleSideEffects,
    updateSession,
    setClaudeSessionId,
...
describe("TaskExecutor.startExecution", () => {
  ...
  it("consumes an observed turn even when its later terminal generation CAS loses", async () => {
    ...
    mocks.enqueueTerminalTransitionAndWaitForApplication.mockResolvedValue({
      eventId: 91,
      applied: false,
      canonicalSession: {
        status: "interrupted",
...
describe("TaskExecutor runner process boundary", () => {
  ...
  it("startNewExecution writes one generation record before running the process", async () => {
    const mocks = makeMocks();
    ...
    expect(dispatcher.prepareExecutionIdentity).toHaveBeenCalledOnce();
    expect(mocks.recordExecutionRegistrationAndWaitForApplication).toHaveBeenCalledOnce();
    expect(mocks.enqueueTerminalTransitionAndWaitForApplication).toHaveBeenCalledOnce();
    expect(task.status).toBe("completed");
  });
  ...
  it.each([
    ...
    "$entryPath 실행은 complete identity proof → registration write 뒤 실행한다",
    async ({ entryPath, expectedTerminalEventId }) => {
      ...
      Object.assign(mocks.persistence, {
        recordExecutionRegistrationAndWaitForApplication: acquire,
        enqueueTerminalTransitionAndWaitForApplication: terminal,
      });
      const { runner, dispatcher } = makeRunnerProcessRuntime([
        { type: "complete", result: "done", timestamp: 1 },
      ]);
  ...
  it("새 스폰은 실행 registration을 기록한다", async () => {
    const mocks = makeMocks();
    const transition = (status: "running" | "completed") => ({
      eventId: status === "running" ? 11 : 12,
      applied: true,
  ...
  it("[A3] clears the settled activation token before a second explicit execution", async () => {
    const mocks = makeMocks();
    const transition = (status: "running" | "completed") => ({
      eventId: status === "running" ? 11 : 12,
      applied: true,
    ...
    const secondExecution = executor.startNewExecution(task, agent);
  ...
  it.each([
    ...
    async ({ executionState, expectedStatus }) => {
      const mocks = makeMocks();
      const recoveredTerminal = mocks.enqueueTerminalTransitionAndWaitForApplication;
  ...
  it("RunnerOrphanedSpawnError before registration cannot feed the dormant child", async () => {
    const mocks = makeMocks();
    const acquire = vi.fn();
    Object.assign(mocks.persistence, {
      recordExecutionRegistrationAndWaitForApplication: acquire,
  ...
  it("실행이 끝나면 execution slot을 비운다", async () => {
    const mocks = makeMocks();
    const { runner } = makeRunnerProcessRuntime([]);
    const executor = new TaskExecutor(
      () => makeFakeEngine([]),
  ...
  it("adopt recovery replays the same runner without recording a new registration", async () => {
    const mocks = makeMocks();
    const transition = (status: "running" | "completed") => ({
    ...
    expect(dispatcher.recoverFrames).toHaveBeenCalledOnce();
  ...
  it("adopt recovered turn registers and consumes a late delivery receipt", async () => {
    const mocks = makeMocks();
    const recovered = deferred<void>();
    const finishRecoveredTurn = deferred<void>();
    const message: InterventionMessage = {
      text: "steer during recovered turn",
      user: "director",
      deliveryId: "adadadad-adad-4dad-8dad-adadadadadad",
      deliveryIntent: "human_live_steer",
    };
    const deliveryRecorder = {
      recordTurnStarted: vi.fn().mockResolvedValue(true),
    ...
    dispatcher.recoverFrames.mockReturnValue((async function* () {
      ...
      yield engineEventFrame({ type: "complete", result: "recovered", timestamp: 1 });
    })());
    const executor = new TaskExecutor(
      () => makeFakeEngine([]),
      mocks.db,
    ...
    expect(task.activeDeliveryTurnReceipt).toBeInstanceOf(TaskDeliveryTurnReceipt);
    await task.activeDeliveryTurnReceipt!.register(message);
    finishRecoveredTurn.resolve();
    await recovery;

    expect(deliveryRecorder.recordTurnStarted).toHaveBeenCalledOnce();
    expect(deliveryRecorder.recordTurnStarted).toHaveBeenCalledWith(message, task);
    expect(deliveryRecorder.recordConsumed).toHaveBeenCalledOnce();
    expect(deliveryRecorder.recordConsumed).toHaveBeenCalledWith(
      message,
    ...
    expect(task.activeDeliveryTurnReceipt).toBeUndefined();
  });
