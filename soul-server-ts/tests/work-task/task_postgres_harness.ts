...
const TEST_DB_NAME = "task_test_db";
const TEST_USER = "task_test";
const TEST_PASSWORD = "task_test";

export interface TaskPostgresHarness {
  sql: SqlClient;
  cleanup(): Promise<void>;
}

export const hasTaskPostgresBackend =
  Boolean(process.env.TEST_DATABASE_URL?.trim()) || hasDockerBinary();

export async function createTaskPostgresHarness(): Promise<TaskPostgresHarness> {
...
export async function resetTaskData(sql: SqlClient): Promise<void> {
  await sql`
    TRUNCATE checklist_task_projection_outbox, pages,
      task_operations, task_items, task_sections, tasks,
...
): Promise<TaskPostgresHarness> {
  const schema = `task_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  const sql = postgres(url, { max: 1, idle_timeout: 1 }) as SqlClient;
  await waitForPostgres(sql);
  await sql.unsafe(`CREATE SCHEMA ${schema}`);
  await sql.unsafe(`SET search_path TO ${schema}`);
  await createTaskSchema(sql);
...
async function createTaskSchema(sql: SqlClient): Promise<void> {
  await sql`
  ...
  await sql.unsafe(`
    ...
    RETURNS TABLE (
      ...
      SELECT
        ...
        membership_kind, source_task_item_id, item_type, item_id, x, y,
  ...
  `;
  await createTaskTables(sql);
}

async function createTaskTables(sql: SqlClient): Promise<void> {
  await sql`
    CREATE TABLE tasks (
      id TEXT PRIMARY KEY,
  ...
  `;
  await sql`CREATE UNIQUE INDEX uq_task_ops_idem ON task_operations(idempotency_key) WHERE idempotency_key IS NOT NULL`;
}
