...
function mutation(snapshot: RunbookSnapshot): RunbookMutationResult {
  return {
    snapshot,
    eventId: 1,
    operation: {
      id: "op-1",
      runbook_id: snapshot.runbook.id,
      target_kind: "item",
      target_id: "target",
      operation_type: "test",
      actor_kind: "agent",
...
function harness() {
  ...
  const port = {
    ...
    createSection: vi.fn(async (params) => {
      ...
      if (!snapshot) throw new Error("runbook missing");
    ...
    patchItem: vi.fn(async (params) => {
      ...
      if (current.version !== params.expectedVersion) {
        throw new RunbookVersionConflict("item", current.id, params.expectedVersion, current.version);
      }
  ...
  } satisfies ChecklistRunbookPort;
...
function item(overrides: Partial<RunbookItemRow> = {}): RunbookItemRow {
  return {
...
describe("ChecklistRunbookAdapter", () => {
  it("adopts legacy checked state into deterministic Runbook objects", async () => {
    const h = harness();
  ...
  it("re-reads and retries a CAS conflict without bypassing RunbookService", async () => {
    const h = harness();
    await h.adapter.reconcile(legacyInput(false));
