...
import { EVENT_DURABILITY, SSE_EVENT_TYPES } from "@soulstream/wire-schema";
...
const SECTION_7_ALL_EVENT_TYPES = SSE_EVENT_TYPES;
const PERSISTENCE_ONLY_EVENT_TYPES = ["metadata"] as const;
const ALL_PERSISTENCE_EVENT_TYPES = [...SSE_EVENT_TYPES, ...PERSISTENCE_ONLY_EVENT_TYPES];
...
describe("EventPersistence transient boundary", () => {
  ...
  it("SSE 밖에서 outbox를 쓰는 내부 이벤트도 같은 분류 정본에 고정한다", () => {
    expect(PERSISTENCE_ONLY_EVENT_TYPES).toHaveLength(1);
