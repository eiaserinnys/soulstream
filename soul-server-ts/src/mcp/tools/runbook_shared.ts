...
import type { RunbookOperationTargetKind } from "../../db/session_db_types.js";
...
export function mutationToolDescription(description: string): string {
  return `${description} 기본 응답은 operation, 변경된 target row, runbook 헤더만 반환한다. 전체 snapshot이 필요하면 include_snapshot=true를 사용한다. ${CALLER_SESSION_ID_GUIDANCE}`;
}
...
export async function mutation(
  ...
  ) => Promise<RunbookMutationEnvelope>,
  options: {
...
}): { assignee?: RunbookAssigneeInput | null } | Record<string, never> {
