...
export interface CodexAdapterConfig {
   ...
   * (`@openai/codex-sdk` dist/index.js:222-231). 공통 child-process sanitizer로
   * billing-switch magic key를 제거하고, Codex 전용 빈 CODEX_API_KEY도 제거한 env만
   * 명시 전달한다.
   */
...
export class CodexEngineAdapter implements EnginePort {
  ...
  constructor(config: CodexAdapterConfig, logger: Logger) {
    ...
    this.codex = new Codex({
      ...
      // env를 명시 전달하면 SDK가 process.env를 상속하지 않는다. 공통 sanitizer의
      // billing-switch key 차단이 실제 Codex CLI 자식까지 유지된다.
