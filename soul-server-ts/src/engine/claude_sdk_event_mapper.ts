...
export class ClaudeSdkEventMapper {
  ...
  mapSystemMessage(message: Record<string, unknown>): ClaudeClientEvent[] {
    if (asString(message.subtype) === "background_tasks_changed") {
      return this.mapBackgroundTaskMembership(message);
    }
  ...
  private mapBackgroundTaskMembership(
    ...
    const transition = this.runtimeState.replaceBackgroundTaskMembership(taskIds);
    const byId = new Map(
      tasks.flatMap((task) => {
    ...
    return transition.started.map((taskId) => {
      ...
      const existing = this.runtimeState.getTaskStatus(taskId);
      this.runtimeState.setTaskStatus(taskId, existing ?? "running");
