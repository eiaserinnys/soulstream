...
import type { ChecklistBlockProperties, ChecklistRunbookReference } from "@soulstream/page-model";

import type { PageYjsHostClient } from "./page_host_client.js";
import type { ChecklistRunbookAdapter } from "./checklist_runbook_adapter.js";
import type {
  ChecklistProjectionOutboxRow,
  ChecklistRunbookProjectionRepository,
} from "./checklist_runbook_projection_repository.js";

export type { ChecklistProjectionOutboxRow } from "./checklist_runbook_projection_repository.js";

interface ChecklistProjectionRepositoryPort {
  claimDue: ChecklistRunbookProjectionRepository["claimDue"];
  markSuccess: ChecklistRunbookProjectionRepository["markSuccess"];
  markFailure: ChecklistRunbookProjectionRepository["markFailure"];
}

interface ChecklistProjectionAdapterPort {
  reconcile: ChecklistRunbookAdapter["reconcile"];
  archive: ChecklistRunbookAdapter["archive"];
}
...
export interface ChecklistRunbookReconcilerDeps {
  nodeId: string;
...
export class ChecklistRunbookReconciler {
  private timer: NodeJS.Timeout | undefined;
  private reconciling = false;

  constructor(private readonly deps: ChecklistRunbookReconcilerDeps) {}
  ...
  async reconcileDue(): Promise<void> {
    ...
    try {
      ...
      this.deps.logger.warn({ err: error }, "checklist Runbook projection scan failed");
    } finally {
  ...
  private async reconcileRow(row: ChecklistProjectionOutboxRow): Promise<void> {
    try {
      ...
      this.deps.logger.warn(
        ...
        "checklist Runbook projection retained for retry",
      );
    }
  }
}
...
function isExactReference(
  ...
  return keys.length === 2
    && keys[0] === "itemId"
    && keys[1] === "runbookId"
    && properties.runbookId === reference.runbookId
