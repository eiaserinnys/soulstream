...
describe("v3 task section navigation contract", () => {
  it("keeps the director-approved task detail section order", () => {
    ...
    const order = ["description", "context", "checklist", "board", "sessions"]
  ...
  it("uses a sticky glass anchor rail with a rounded focus ring and a narrow-width cutoff", () => {
    const detail = read("./TaskDetailPane.tsx");
    ...
    expect(navigation).toContain("glass-strong glass-chrome lg-rim");
    expect(navigation).not.toMatch(/collaps|접기|펼치기/i);
    expect(css).toMatch(/\.v3-task-section-nav\s*\{[^}]*position:\s*sticky/s);
    expect(css).toMatch(/\.v3-task-detail-layout\s*\{[^}]*grid-template-columns:\s*76px\s+minmax\(0,\s*1fr\)/s);
