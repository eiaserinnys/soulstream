/**
 * node-guard.ts - 다른 노드 세션 판별 로직 (unified-dashboard)
 *
 * soul-dashboard의 node-guard.ts에서 포팅.
 * DashboardLayout에서 활성 세션이 다른 노드 소속인지 판단한다.
 * 순수 함수로 분리하여 독립적으로 테스트 가능하다.
 *
 * unified-dashboard에서는 AppConfig.features.nodeGuard가 true일 때만
 * /api/node-info를 fetch하여 currentNodeId를 설정한다.
 */
