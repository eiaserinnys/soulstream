...
const outputRoot = path.resolve(
  process.env.PR_CF_QA_OUTPUT ?? path.join("e2e", "screenshots", "v3-pr-cf-task-crud"),
);
const strict = process.env.PR_CF_QA_STRICT === "1";

const result = await runPlaywrightLifecycle({
  lockName: `pr-cf-task-crud-${strict ? "after" : "before"}`,
  timeoutMs: 180_000,
}, async ({ browser }) => {
  const themes = [];
...
async function verifyTheme(browser: Browser, theme: Theme) {
  ...
  try {
    ...
    const card = page.getByTestId("v3-task-checklist").getByTestId("task-card");
    await card.waitFor({ state: "visible", timeout: 30_000 });
    await card.getByText("요구사항 확인", { exact: true }).waitFor({ state: "visible" });
    await capture(page, theme, "01-checklist-layout");

    const firstRow = card.getByTestId("task-item-row").filter({ hasText: "요구사항 확인" });
    const detailsToggle = firstRow.getByTestId("task-item-details-toggle");
    const initialMetrics = await card.evaluate((element) => ({
      itemRows: element.querySelectorAll('[data-testid="task-item-row"]').length,
      borderedItemRows: Array.from(element.querySelectorAll<HTMLElement>('[data-testid="task-item-row"]'))
        .filter((row) => row.className.includes("border")).length,
      openDetails: element.querySelectorAll('[data-testid="task-how-to"]').length,
      rowMenus: element.querySelectorAll('[data-testid="task-row-menu"]').length,
    }));
    ...
    if (strict) {
      ...
      assert(
        await firstRow.getByTestId("task-how-to").evaluate((element) => element.className.includes("border-l-2")),
...
async function exerciseCrud(
  ...
  const sectionCreate = card.getByTestId("task-section-editor");
  ...
  const sectionUpdate = card.getByTestId("task-section-editor");
  ...
  await expectOrder(card, '[data-testid="task-section-toggle"]', ["준비", "배포 준비", "검수"]);

  const planSection = sectionByTitle(card, "준비");
  await planSection.getByRole("button", { name: "항목 추가" }).click();
  const itemCreate = planSection.getByTestId("task-item-editor");
  ...
  const itemUpdate = card.getByTestId("task-item-editor");
  ...
  await page.getByRole("menuitem", { name: "위로 이동" }).click();
  await expectOrder(planSection, '[data-testid="task-item-title"]', [
    "요구사항 확인",
    "릴리스 노트 확정",
    "구현 검증",
  ]);
...
function sectionByTitle(card: ReturnType<Page["getByTestId"]>, title: string) {
  ...
  return card.getByTestId("task-section-toggle")
    .filter({ hasText: exactTitle })
    .locator("xpath=ancestor::*[@data-testid='task-section']");
}
...
async function openItemMenu(
  ...
  const row = card.getByTestId("task-item-row").filter({ hasText: title });
  await row.hover();
