...
async function verifyTheme(browser: Browser, theme: "dark" | "light") {
  ...
  try {
    ...
    const fixedCard = page.getByTestId("task-board-fixed-card");
    ...
    assert(before.fixed.width === 360 && before.fixed.height === 520, `고정 업무 카드가 ${before.fixed.width}×${before.fixed.height}px입니다.`);
    assert(before.fixedOverlaps > 0, "정리 전 업무 카드 겹침을 재현하지 못했습니다.");
    ...
    await page.waitForFunction(() => {
      const fixed = document.querySelector<HTMLElement>('[data-testid="task-board-fixed-card"]')?.getBoundingClientRect();
    ...
    assert(after.fixedOverlaps === 0, `정리 뒤 업무 카드와 ${after.fixedOverlaps}개 카드가 겹칩니다.`);
...
async function measureLayout(page: Page) {
  const layout = await page.evaluate(() => {
    const fixedElement = document.querySelector<HTMLElement>('[data-testid="task-board-fixed-card"]');
    if (!fixedElement) throw new Error("고정 업무 카드를 찾지 못했습니다.");
