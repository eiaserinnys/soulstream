...
const outputRoot = path.resolve(
  process.env.PR_BL_QA_OUTPUT ?? path.join("e2e", "screenshots", "v3-task-runbook-checklist"),
);

const result = await runPlaywrightLifecycle({
  lockName: "pr-bl-v3-task-runbook-checklist",
  timeoutMs: 180_000,
}, async ({ browser }) => ({
...
async function verifyTheme(browser: Browser, theme: "dark" | "light") {
  ...
  const fixture = createRunbookFixture();
  ...
  try {
    ...
    const runbookCard = checklist.getByTestId("runbook-card");
    await runbookCard.waitFor({ state: "visible", timeout: 20_000 });
    await runbookCard.getByTestId("runbook-item-row").first().waitFor({ state: "visible" });
    const renderMs = Math.round(performance.now() - renderStartedAt);

    assert(await checklist.getByTestId("runbook-card").count() === 1, "업무 창이 공통 RunbookCard를 재사용하지 않았습니다.");
    assert(await runbookCard.getByTestId("runbook-card-progress").innerText() === "8/48", "48항목 진행률이 다릅니다.");
    assert(await runbookCard.getByTestId("runbook-section-toggle").count() === 2, "런북 섹션 수가 다릅니다.");
    assert(await runbookCard.getByTestId("runbook-item-row").count() === 40, "완료 섹션이 기본으로 접히지 않았습니다.");
    assert(!await runbookCard.getByText("완료 항목 1", { exact: true }).isVisible(), "완료 항목이 기본 접기 정책을 벗어났습니다.");

    const scrollMetrics = await runbookCard.evaluate((element) => {
    ...
    assert(scrollMetrics !== null, "런북 내부 스크롤 영역을 찾지 못했습니다.");
    assert(scrollMetrics.scrollHeight > scrollMetrics.clientHeight, "48항목 런북이 업무 창 전체를 밀어내고 있습니다.");
    assert(scrollMetrics.cardHeight <= 520, `런북 카드 높이가 상한을 넘었습니다: ${scrollMetrics.cardHeight}px`);

    const firstToggle = runbookCard.getByTestId("runbook-status-toggle").first().locator('input[type="checkbox"]');
    assert(!await firstToggle.isChecked(), "검증 항목의 초기 상태가 완료입니다.");
    await firstToggle.click();
    await page.waitForFunction(() => {
      const input = document.querySelector<HTMLInputElement>('[data-testid="runbook-status-toggle"] input[type="checkbox"]');
    ...
    assert(await runbookCard.getByTestId("runbook-card-progress").innerText() === "9/48", "완료 처리 뒤 진행률이 갱신되지 않았습니다.");

    const visibleText = await checklist.innerText();
    assert(!visibleText.includes("rb-alpha"), "런북 내부 식별자가 업무 창에 노출되었습니다.");
    ...
    await capture(page, theme, "01-task-runbook-checklist");
...
function createRunbookFixture() {
  ...
  const install = async (page: Page) => {
    await page.route("**/api/runbooks/rb-alpha/items/active-item-1/status", async (route) => {
...
async function preparePage(
  page: Page,
  theme: "dark" | "light",
  fixture: ReturnType<typeof createRunbookFixture>,
) {
