"""terminal 상태(completed/error/interrupted)에서 후속 메시지 시 Claude SDK를 fresh로 시작.

핵심 정책 (코덱스 진단 atom 0fa49771):
- terminal 분기에서 submit_message가 skip_claude_resume=True 적용 → task.resume_session_id=None
- task_executor._run_execution이 engine_adapter.execute에 resume_session_id=None을 forward
- engine_adapter는 client_lifecycle.build_options에서 ClaudeAgentOptions.resume을 미설정
- → Claude SDK가 previous_message_id 체인 없이 새 conversation 시작
- → Claude 계정 limit 후에도 400 에러 없이 정상 처리

본 테스트는 task_executor가 *engine_adapter에 resume_session_id=None을 forward*하는지를
직접 검증하여 fix의 wire 끝까지의 정합을 보호한다.
"""
...
class TestTerminalResumeFreshStart:
    """terminal-resume 후 task_executor → engine_adapter wire에서 resume_session_id=None 보존."""
    ...
    async def test_resume_session_id_none_forwarded_to_engine(
        self, mock_broadcaster, mock_assemble, mock_build
    ):
        """task.resume_session_id가 None이면 engine_adapter.execute가 resume_session_id=None을 받는다.

        이는 client_lifecycle.build_options L256(`if session_id: options.resume = session_id`)
        에서 if False 분기 → ClaudeAgentOptions.resume 미설정 → Claude SDK fresh 시작.
        """
        ...
        db = _make_db()
        # ★ terminal-resume 후의 상태 — claude_session_id 보존, resume_session_id=None
        task = _make_task(
        ...
        # ★ 핵심: engine_adapter.execute가 resume_session_id=None을 받음
    ...
    async def test_resume_session_id_set_when_skip_claude_resume_false(
        ...
        """대조군: skip_claude_resume이 적용되지 *않은* 경로(직접 create_task 호출 등)에서는
        기존 동작 보존 — resume_session_id가 engine에 forward된다.

        본 fix는 submit_message terminal 분기에만 skip_claude_resume=True를 박는다.
        그 외 직접 호출 경로는 영향 없음을 검증한다.
        """
        mock_broadcaster.return_value = MagicMock(
            emit_session_updated=AsyncMock(),
            emit_session_message_updated=AsyncMock(),
