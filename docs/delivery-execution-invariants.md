...
`steady-state`의 일반 턴과 개입 턴에서 관측한 호출 결과, 이벤트 순서, 메시지·도구 payload, 응답 수, 종료 상태, 가시 오류 집합을 각각 기준선으로 삼는다. `restart-adopt`는 일반 턴 기준선과, `restart-intervention-window`는 개입 턴 기준선과 대조한다. 시간과 실행별 식별자만 판정에서 제외한다. 나머지 관측이 하나라도 다르면 재시작을 추론할 수 있으므로 실패다.
