...
기준선의 내용은 현재 구현을 관측해 복사하지 않는다. 설계 단계에서 사용자 요구로 먼저 고정한다. 일반 턴은 입력 1회, 도구 시작·정상 결과 각 1회, 최종 응답 1회, 완료, 가시 실패 0이다. 개입 턴은 입력 1회, 도구 시작, 개입 접수 1회, 도구 정상 결과, 개입과 기존 컨텍스트가 반영된 최종 응답 1회, 완료, 호출·가시 실패 0이다. Claude의 호출 ACK는 `queued_for_next_turn`, Codex는 `delivered`가 정상 계약이다.
