...
대부분의 경우 사용자와 실행 중인 에이전트는 서버가 재시작됐다는 사실을 알 수 없어야 한다. 재시작 동안 대시보드가 내려가는 것과 전달 지연만 허용한다. 그 밖의 사용자·에이전트 가시 신호가 평상시와 달라지면 실패다. 다른 경로로 재시작 사실이나 우회 방법을 안내하는 것은 성공의 대체물이 아니다.

따라서 다음은 모두 실패로 판정한다.

- 에러 응답·거절·503. 내구 접수 뒤의 거짓 실패뿐 아니라 실제 실패도 사용자에게 보였다면 실패다.
- 재전송·재시도 요구, 재시작·복구 중임을 알리는 안내.
- 진행 중인 턴 중단, 도구 실행 결과 소실·변경, 응답 누락·중복.
- 에이전트가 관측하는 예외, 상태 리셋, 컨텍스트 유실.

기준선의 내용은 현재 구현을 관측해 복사하지 않는다. 설계 단계에서 사용자 요구로 먼저 고정한다. 일반 턴은 입력 1회, 도구 시작·정상 결과 각 1회, 최종 응답 1회, 완료, 가시 실패 0이다. 개입 턴은 입력 1회, 도구 시작, 개입 접수 1회, 도구 정상 결과, 개입과 기존 컨텍스트가 반영된 최종 응답, 완료, 호출·가시 실패 0이다. Claude의 `durable_next_turn`은 현재 턴 응답과 다음 턴의 개입 반영 응답이 각각 정확히 한 번이고 호출 ACK는 `queued_for_next_turn`이어야 한다. Codex는 현재 턴의 최종 응답에 개입이 반영되고 ACK는 `delivered`여야 한다.

`steady-state`도 이 작성된 명세와 대조한다. 현재 통과 여부는 명세의 근거가 아니다. `restart-adopt`는 같은 일반 턴 명세와, `restart-intervention-window`는 같은 개입 턴 명세와 대조한다. ①의 실제 관측과 ②·③의 실제 관측 차이는 진단 증거로 함께 남기되 통과 기준을 대신하지 않는다. 시간과 실행별 식별자만 판정에서 제외한다. 나머지 의미 관측이 하나라도 다르면 재시작을 추론할 수 있으므로 실패다.
...
11. 진행 중인 턴의 재시작 복구가 완료되기 전에 접수한 개입은 사용자 재전송 없이 정확히 한 번 실행 턴에 반영되어야 한다. 호출자는 실패를 보지 않아야 하며, 호출 응답·세션 메시지·도구 결과·컨텍스트·종료 상태는 평상시 개입과 구분 불가능해야 한다.
...
| 재시작 복구 전 개입 | identity-proven→active 복구 창에 접수된 개입을 내구 보존하고 복구된 턴에 정확히 한 번 귀속한다 | `restart-intervention-window` |
...
- 세 시나리오는 회귀 확인이 아니라 재설계 명세다. 현재 GREEN은 우연히 이미 만족한다는 부수 정보이고, 현재 RED는 재설계가 GREEN으로 만들 대상이다. 현재 동작을 근거로 기대값을 고치지 않는다.
- `steady-state`, `restart-adopt`, `restart-intervention-window`를 장애 주입 시나리오보다 먼저 실행한다. 정상 기준선이 깨졌으면 나머지 결함 시나리오의 판정을 신뢰하지 않는다.
- `restart-adopt`와 `restart-intervention-window`는 단순한 최종 GREEN이 아니라 위 수용 기준의 정상 관측과 구조적으로 동일해야 한다. 지연만 허용한다.
- `restart-intervention-window`는 adoption 활성화 직전의 DB 대기 상태를 관측한 뒤 단 한 번 개입한다. 무작정 재시작 직후 전송하거나 실패 후 재시도해 창과 유실을 가리지 않는다.
