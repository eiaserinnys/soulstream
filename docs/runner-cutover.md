...
5. MCP를 쓰는 soul-server는 `MCP_ENABLED=true`, `MCP_STATELESS_TRANSPORT_ENABLED=true`, production auth 설정을 먼저 적용한다. `MCP_INTERNAL_PORT`는 명시하거나 `PORT+1` 파생값을 사용하되, nginx 설정에 이 포트를 추가하지 않았는지 확인한다. LLM 클라이언트는 public listener의 stateless `/mcp`, 러너의 Claude SDK를 포함한 내부 소비자는 별도 loopback listener의 stateful `/mcp/internal`로 분리된다. 내부 SDK는 host 재시작 뒤 러너 재연결·새 turn에서 MCP initialize를 다시 수행한다.
