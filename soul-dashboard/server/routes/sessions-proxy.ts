...
export function createSessionsProxyRouter(
   ...
   *
   * Soul Server의 /sessions/:id/history SSE를 프록시합니다.
   * Last-Event-ID 헤더를 전달합니다.
   */
  router.get("/:id/events", async (req: Request, res: Response) => {
    try {
      ...
      // URL 구성 (Last-Event-ID를 쿼리 파라미터로 전달)
      const url = new URL(
      ...
      try {
        soulResponse = await fetch(url.toString(), {
          ...
          headers: {
            Accept: "text/event-stream",
            ...(authToken ? { Authorization: `Bearer ${authToken}` } : {}),
            // Last-Event-ID 헤더도 전달 (일부 서버가 헤더를 기대할 수 있음)
        ...
        console.error(
          "[sessions-proxy] Failed to connect to history SSE stream:",
          err,
        );
        res.status(502).json({
          error: {
      ...
      if (!soulResponse.ok) {
        ...
        if (contentType.includes("application/json")) {
          ...
          console.error(
            `[sessions-proxy] Soul GET /sessions/:id/history failed (${soulResponse.status}):`,
            errorText,
          );
          res.status(soulResponse.status).json({
            error: {
      ...
      console.error(
        "[sessions-proxy] Failed to proxy GET /sessions/:id/events:",
        err,
      );
      if (!res.headersSent) {
        res.status(500).json({
